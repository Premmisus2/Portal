...
export default async function handler(req: any, res: any) {
  ...
  if (!SB_KEY || !BOT_TOKEN || !CHAT_ID) return res.status(500).json({ error: 'Not configured' });
  ...
  try {
    ...
    if (!Array.isArray(logs)) return res.status(200).json({ message: 'No logs' });
    ...
    return res.status(200).json({ success: teleData.ok, totalCalls, totalBookings });
