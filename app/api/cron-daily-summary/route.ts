...
export async function GET(request: Request) {
  ...
  try {
    ...
    // ── Login roll-up ─────────────────────────────────────────────────────
    // Pulls today's auth events from the audit_log and reports who signed in
    // and who didn't. Counts both `auth.signed_in` and `auth.registered` —
    // a registration creates a session, so it counts as "logged in today."
    ...
    try {
      ...
      if (Array.isArray(authEvents)) {
        const firstByRep = new Map<string, { time: Date; isSignup: boolean }>();
        for (const ev of authEvents) {
          if (!ev.actor_rep_id || firstByRep.has(ev.actor_rep_id)) continue;
          firstByRep.set(ev.actor_rep_id, {
            time: new Date(ev.created_at),
            isSignup: ev.action_type === 'auth.registered',
          });
        }
        summary += `\n\n*🔓 LOGINS — ${firstByRep.size} of ${activeReps.length}*\n`;
        for (const rep of activeReps) {
          const ev = firstByRep.get(rep.id);
          if (ev) {
            const t = ev.time.toLocaleTimeString('en-US', { timeZone: 'America/Toronto', hour: 'numeric', minute: '2-digit', hour12: true });
            summary += `  ${ev.isSignup ? '🆕' : '✅'} ${rep.name} — ${t} ET\n`;
          } else {
            summary += `  ❌ ${rep.name} — did not log in\n`;
          }
        }
      }
    } catch {
      // Login roll-up is best-effort. The call summary above is the primary signal.
    }
