    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', marginBottom: '20px' }}>
        <div className="card-glow" style={{ padding: '16px 18px' }}>
          <p style={{ fontSize: '9px', fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--accent-ink)', margin: '0 0 6px' }}>Total Calls</p>
        ...
        <div className="card-glow" style={{ padding: '16px 18px' }}>
          ...
          <p style={{ fontSize: '26px', fontWeight: 900, color: 'var(--accent-ink)', margin: 0, fontFamily: 'monospace' }}>{bookedCalls}</p>
        </div>
      </div>
      ...
      <button onClick={() => setShowForm(!showForm)} style={{
        ...
        border: '1px solid var(--accent-glow-40)', background: 'var(--accent-glow-05)', color: 'var(--accent-ink)',
      ...
      {showForm && (
        <div className="card-glow" style={{ padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 800, color: 'var(--text-primary)', margin: '0 0 16px' }}>Log Call</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Business Name</label>
              <input className="field" value={callBiz} onChange={e => setCallBiz(e.target.value)} placeholder="e.g. King Quality Cleaning" />
            </div>
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Niche</label>
              <select className="field" value={callNiche} onChange={e => setCallNiche(e.target.value)}>
                {niches.map(n => <option key={n} value={n}>{n}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Outcome</label>
              <select className="field" value={callOutcome} onChange={e => setCallOutcome(e.target.value)}>
                {outcomes.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Call Date</label>
              <input className="field" type="date" value={callDate} onChange={e => setCallDate(e.target.value)} />
            </div>
          </div>
          <div style={{ marginBottom: '12px' }}>
            <label style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '6px', display: 'block' }}>Notes</label>
          ...
          <button onClick={saveCall} disabled={saving || !callBiz.trim()} style={{
            ...
            border: 'none', background: saving ? 'var(--text-faint)' : 'var(--accent-ink)', color: 'var(--bg-app)',
      ...
      {/* Call History */}
      <h3 style={{ fontSize: '14px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '12px' }}>Call History</h3>
      {allLogs.length === 0 ? (
        <div className="card" style={{ padding: '32px', textAlign: 'center' }}>
          <p style={{ color: 'var(--text-faint)', fontSize: '14px', margin: 0 }}>No calls logged yet. Click &quot;Log a Call&quot; to start tracking.</p>
        </div>
      ) : (
              ...
              <div key={log.id || i} className="card" style={{ padding: '14px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: outcomeColors[oc] || 'var(--text-muted)', boxShadow: `0 0 6px ${outcomeColors[oc] || 'var(--text-muted)'}55` }} />
                    <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>{log.business_name || (log as any).leads?.business_name || '—'}</span>
                    <span style={{ fontSize: '11px', color: 'var(--text-faint)' }}>{(log as any).niche}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: 'var(--text-faint)' }}>{new Date(log.created_at).toLocaleDateString(undefined, { timeZone: 'America/Toronto' })}</span>
                  </div>
                </div>
                {log.notes && <p style={{ margin: '8px 0 0', fontSize: '12px', color: 'var(--text-muted)', lineHeight: 1.5 }}>{log.notes}</p>}
              </div>
            );
          })}
        </div>
