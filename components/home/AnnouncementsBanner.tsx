    ...
    <div style={{maxWidth:'1100px', margin:'0 auto', width:'100%', padding:'0 16px'}}>
      {announcements.map((a: any) => (
        <div key={a.id} className="fadein" style={{
          ...
          background: a.priority === 'urgent' ? 'rgba(255,136,0,.06)' : 'var(--accent-glow-04)',
          border: `1px solid ${a.priority === 'urgent' ? 'rgba(255,136,0,.25)' : 'var(--accent-glow-15)'}`,
          display:'flex', alignItems:'flex-start', gap:'12px',
        }}>
          <div style={{width:'6px', height:'6px', borderRadius:'50%', marginTop:'6px', flexShrink:0,
            background: a.priority === 'urgent' ? '#ff8800' : 'var(--accent-ink)',
            boxShadow: `0 0 8px ${a.priority === 'urgent' ? 'rgba(255,136,0,.6)' : 'rgba(0,240,255,.6)'}`}}/>
          <div style={{flex:1}}>
            <div style={{display:'flex', alignItems:'center', gap:'8px', marginBottom:'4px'}}>
              <span style={{fontSize:'12px', fontWeight:800, color:'var(--text-primary)'}}>{a.title}</span>
              ...
              <span style={{fontSize:'9px', color:'var(--text-faint)', fontFamily:'monospace', marginLeft:'auto'}}>
                {new Date(a.created_at).toLocaleDateString('en-CA', { month:'short', day:'numeric', timeZone: 'America/Toronto' })}
              </span>
            </div>
            <p style={{margin:0, fontSize:'12px', color:'var(--text-tertiary)', lineHeight:1.5}}>{a.message}</p>
          </div>
        </div>
      ))}
    </div>
