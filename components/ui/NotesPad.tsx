    ...
    <div className="no-print" style={{marginTop:'32px', border:`1px solid ${borderColor}`, borderRadius:'10px', overflow:'hidden', transition:'border-color .3s'}}>
      <button onClick={() => setOpen(p => !p)} style={{width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 16px', background:'var(--bg-elev-2)', border:'none', cursor:'pointer', outline:'none', transition:'background .2s'}}
        ...
        <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
          ...
          {!hasNotes && <span style={{fontSize:'10px', color:'var(--text-faint)'}}>— notes save for {NOTES_EXPIRY_DAYS} days then auto-clear</span>}
        </div>
        <span style={{fontSize:'12px', color:'var(--text-faint)', display:'inline-block', transform:open?'rotate(90deg)':'rotate(0deg)', transition:'transform .2s'}}>▸</span>
      </button>
      {open && (
        <>
          ...
          <div style={{padding:'10px 14px', background:'var(--bg-nav)', borderTop:'1px solid var(--bg-sidebar-line)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'10px', flexWrap:'wrap'}}>
            ...
            {uploadResult && uploadResult !== 'error' && (
              <span style={{fontSize:'10px', color:'var(--green)', fontFamily:'JetBrains Mono, monospace'}}>
                ✓ Saved — <a href={uploadResult.url} target="_blank" rel="noreferrer" style={{color:'var(--accent-ink)'}}>Open in Google Docs</a>
              </span>
            )}
            {uploadResult === 'error' && (
              <span style={{fontSize:'10px', color:'var(--red-strong)', fontFamily:'JetBrains Mono, monospace'}}>Upload failed — try again</span>
            )}
          </div>
        </>
      )}
