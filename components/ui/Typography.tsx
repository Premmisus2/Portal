  ...
  return (
    <div className="rep-block">
      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'6px'}}>
        ...
        <button onClick={handleCopy} style={{background:'transparent', border:'1px solid #1e1e1e', borderRadius:'5px', padding:'3px 8px', cursor:'pointer', color:copied?'#00F0FF':'#333', display:'flex', alignItems:'center', gap:'4px', fontSize:'10px', fontWeight:700, letterSpacing:'.08em', fontFamily:'Inter,sans-serif', transition:'color .15s, border-color .15s', flexShrink:0}}
          onMouseEnter={e=>{e.currentTarget.style.borderColor='#444'; e.currentTarget.style.color='#aaa';}}
          onMouseLeave={e=>{if(!copied){e.currentTarget.style.borderColor='#1e1e1e'; e.currentTarget.style.color='#333';}}}>
...
export const SubH = ({ children }: { children: ReactNode }) => (
  <h4 style={{margin:'10px 0 5px',fontSize:'15px',fontWeight:700,color:'#fff',lineHeight:1.3}}>{children}</h4>
);

/* ─── Body Text ─── */
export const Body = ({ children }: { children: ReactNode }) => (
  <p style={{color:'#888',fontSize:'14px',lineHeight:1.6,margin:'4px 0'}}>{children}</p>
);
...
export const Li = ({ children }: { children: ReactNode }) => (
  <li style={{display:'flex',alignItems:'flex-start',gap:'10px',padding:'3px 0',listStyle:'none'}}>
    <span style={{width:'5px',height:'5px',borderRadius:'50%',background:'#00F0FF',flexShrink:0,marginTop:'8px'}}/>
    <span style={{color:'#aaa',fontSize:'14px',lineHeight:1.55}}>{children}</span>
  </li>
);
