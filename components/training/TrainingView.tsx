    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-app)', paddingTop: shadowMode ? '40px' : 0 }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: '480px', padding: '40px 20px' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'var(--accent-glow-06)', border: '1px solid var(--accent-glow-22)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
            <Icon name="zap" size={28} />
          </div>
          <span className="tag" style={{ marginBottom: '16px', display: 'inline-block' }}>Coming Soon</span>
          <h2 style={{ fontSize: '28px', fontWeight: 900, color: 'var(--text-primary)', margin: '12px 0 16px', letterSpacing: '-.01em' }}>Training Hub</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', lineHeight: 1.65, margin: '0 0 24px' }}>
